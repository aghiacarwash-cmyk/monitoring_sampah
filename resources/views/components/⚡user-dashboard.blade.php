...
<div>
    ...
    <script>
        // Simpan instance map di window supaya tidak hilang/konflik antar re-render Livewire
        window.cleanIotMap = window.cleanIotMap || null;
 
        function initCleanIotMap() {
            const mapDiv = document.getElementById('map');
            if (!mapDiv) return;
 
            // Hapus instance lama kalau ada, supaya tidak konflik dengan DOM baru
            if (window.cleanIotMap) {
                window.cleanIotMap.remove();
                window.cleanIotMap = null;
            }
 
            window.cleanIotMap = L.map('map');
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '&copy; OpenStreetMap'
            }).addTo(window.cleanIotMap);
            ...
            if (bounds.length > 0) {
                window.cleanIotMap.fitBounds(bounds, { padding: [40, 40] });
            } else {
                window.cleanIotMap.setView([-6.2, 106.8], 11);
            }
 
            // Paksa Leaflet hitung ulang ukuran container, mencegah tile putih/abu-abu
            setTimeout(() => {
                window.cleanIotMap.invalidateSize();
            }, 100);
        }
 
        // Jalankan saat load pertama kali
        document.addEventListener('DOMContentLoaded', initCleanIotMap);
 
        // Jalankan ulang setiap kali Livewire selesai re-render komponen ini
        document.addEventListener('livewire:init', () => {
            Livewire.hook('morph.updated', ({ el }) => {
                if (el.id === 'map' || (el.querySelector && el.querySelector('#map'))) {
                    initCleanIotMap();
                }
            });
        });
 
        // Fallback: kalau script ini dieksekusi setelah DOMContentLoaded sempat lewat
